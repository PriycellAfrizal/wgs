...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Fungsi buatpo
function buatpo() {
    var selectedRows = document.querySelectorAll('.selected'); // Ambil semua baris yang dipilih
    ...
    selectedRows.forEach(function(rowSelect, index) {
        var row = rowSelect;
        var no = row.children[1].textContent.trim();
        ...
        <input type="text" class="price" autocomplete="off" id="price${index}" required name="price" oninput="formatOnInput(this, ${index})">
        ...
        // Tambahkan baris baru ke dalam tabel PO

        var newRow = `<tr>
                        <td>${index + 1}</td>
                        <td>${nosp} </td>
                        <td>${namabarang}  </td>
                        <td style="text-align: center;"><input type="text" name="qty" id="qty${index}" value="${qty}" style="width: 50px; text-align: center;" oninput="updateTotalPrice(this, ${index})" data-buatpoqty="${parseFloat(qty)}"></td>
                        <td>${satuan}</td>
                        <td style="display: none;"> ${divisi}</td> <!-- Kolom tersembunyi -->
                        <td style="text-align: center;">${price}</td> <!-- Kolom harga -->

                        <td style="text-align: center; display: none; width: 50px;">${discount}</td> <!-- Kolom diskon -->

                              <td style="display: none;">    ${discountAmount}</td> <!-- Kolom discountAmount -->


                        <td class="total-price-cell">${totalprice}</td> <!-- Kolom total harga -->
                        
                              <td style="display:none;">${kodeproduksi}</td> <!-- Kolom SPK -->
                              <td style="display:none;">${spk}</td> <!-- Kolom SPK -->
...
function updateTotalAll() {
    ...
    let grandTotal = subtotal + ppnAmount - pphAmount;
...
function savePo() {
    ...
    tableRows.forEach(row => {
        ...
        const spk = row.cells[10].textContent.trim();
        const kodeproduksi = row.cells[11].textContent.trim();
        const buatpoQty = parseFloat(qtyInput.dataset.buatpoqty);
        ...
        if (!qty || !price) {
            ...
            qtyInput.setCustomValidity('Wajib diisi.');
            priceInput.setCustomValidity('Wajib diisi.');
        } else {
            qtyInput.setCustomValidity('');
    ...
    if (qtyExceeded) {
        const exceeded = items.find(item => parseFloat(item.qty) > parseFloat(item.buatpoQty));
        alert(`Qty untuk NO SP ${exceeded.nosp} (${exceeded.namabarang}) tidak boleh melebihi ${exceeded.buatpoQty}.`);
        return;
    }
    ...
    Promise.all(priceCheckPromises).then(() => {
        function kirimDataKeServer() {
            ...
            $.ajax({
                type: 'POST',
                url: 'purchaselocal/save_planning.php',
        ...
        if (perubahanHargaItems.length > 0) {
            ...
            Swal.fire({
                ...
                html: notif + '<br>Lanjutkan penyimpanan?',
                icon: 'warning',
...
function showNotification() {
    Swal.fire({
        title: 'Berhasil!',
        text: 'Planning Berhasil di buat!',
        icon: 'success',
        timer: 3000,
